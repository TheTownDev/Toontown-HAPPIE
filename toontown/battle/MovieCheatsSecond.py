...
def doFieldPromotion(scene, battle):
    ...
    promotion_movie = Parallel(Sequence(Wait(1), Parallel(Func(fire_sfx.play), ActorInterval(lucky_suit, 'powered-up'), particalTrackFull, Func(lucky_suit.getGeomNode().setColorScale, Vec4(0.9, 0.65, 0.65, 1.0))), Func(lucky_suit.loop, 'neutral'), Func(lucky_suit.getGeomNode().setColorScale, Vec4(1.0, 1.0, 1.0, 1.0))))
