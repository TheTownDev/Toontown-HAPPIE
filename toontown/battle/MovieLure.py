...
def __createFishingPoleMultiTrack(lure, dollar, dollarName, dollarType):
    ...
    if sidestep == 0:
        if kbbonus == 1 or hp > 0:
            ...
            if suit.dna.body == 'a':
                if suit.getSz() > 1.0:
                    retardPos.setY(retardPos.getY() + ((MovieUtil.SUIT_EXTRA_REACH_DISTANCE - .105) + suit.getSz()))
                else:
                    retardPos.setY(retardPos.getY() + MovieUtil.SUIT_EXTRA_REACH_DISTANCE)
            else:
                if suit.getSz() > 1.0:
                    retardPos.setY(retardPos.getY() + (MovieUtil.SUIT_EXTRA_REACH_DISTANCE + (suit.getSz() * 2)))
                else:
                    retardPos.setY(retardPos.getY() + MovieUtil.SUIT_EXTRA_REACH_DISTANCE)
...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp):
    ...
    if trapName == 'banana':
        ...
        animTrack = Sequence(ActorInterval(suit, 'flail'), ActorInterval(suit, 'flail', startTime=1.1), Wait(0.7), ActorInterval(suit, 'slip-forward'))
        ...
        animTrack = Sequence(getSplicedLerpAnimsTrack(suit, 'flail', 0.7, 0.25), Func(trapProp.setColor, Vec4(0, 0, 0, 1)), ActorInterval(suit, 'flail', startTime=0.7, endTime=0), ActorInterval(suit, 'neutral', duration=0.5), ActorInterval(suit, 'flail', startTime=1.1), Wait(1.1), ActorInterval(suit, 'slip-forward'))
        damageTrack = Sequence(Wait(3.5), Func(suit.showHpText, -hp, openEnded=0), Func(suit.updateHealthBar, hp))
