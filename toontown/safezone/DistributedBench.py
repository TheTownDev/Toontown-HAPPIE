...
class DistributedBench(DistributedObject.DistributedObject):
    seatState = IntEnum('seatState', ('Empty', 'Full', 'Eating'))
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedBench')
    ...
    def announceGenerate(self):
        ...
        for i in range(self.numSeats):
            self.seats.append(self.picnicTable.find('**/loc_seat%d' % (i + 1)))
            self.jumpOffsets.append(self.picnicTable.find('**/loc_jumpOff%d' % (i + 1)))

        self.tablecloth = self.picnicTable
        DistributedObject.DistributedObject.announceGenerate(self)
        for i in range(self.numSeats):
            self.picnicTableSphereNodes.append(self.seats[i].attachNewNode(CollisionNode('bench_sphere_%d_%d' % (self.getDoId(), i))))
        ...
        unitVec = Vec3(math.cos(radAngle), math.sin(radAngle), 0) #LerpPosHprInterval(camera, 2.0, Point3(1.92, 21.80, 3.80), Point3(180, 0, 0))
        unitVec *= 30.0
        ...
        if avId == 0:
            ...
            if avId in self.cr.doId2do:
                ...
                if self.fullSeat.count(0) == 3:
                    ...
                    self.picnicBasketTrack = Sequence(Wait(waitDuration))
    ...
    def __enableCollisions(self):
        for i in range(self.numSeats):
            self.accept('enterbench_sphere_%d_%d' % (self.getDoId(), i), self.handleEnterPicnicTableSphere, [i])
            self.accept('enterBenchOK_%d_%d' % (self.getDoId(), i), self.handleEnterPicnicTable, [i])
            self.picnicTableSphereNodes[i].setCollideMask(ToontownGlobals.WallBitmask)

    def __disableCollisions(self):
        for i in range(self.numSeats):
            self.ignore('enterbench_sphere_%d_%d' % (self.getDoId(), i))
            self.ignore('enterBenchOK_%d_%d' % (self.getDoId(), i))
    ...
    def generateToonJumpTrack(self, av, seatIndex):
        ...
        def getToonJumpTrack(av, seatIndex):
            ...
            def getJumpDest(av = av, node = self.tablecloth):
                ...
                seatNode = self.picnicTable.find('**/loc_seat' + str(seatIndex + 1))
