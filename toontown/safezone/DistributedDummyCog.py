...
class DistributedDummyCog(DistributedObject, FSM):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedDummyCog')
    ...
    def setCogProperties(self):
        ...
        self.bodyCollNode = CollisionNode('ToonfestCogBodyCollision-' + str(self.doId))
        ...
        self.headCollNode = CollisionNode('ToonfestCogHeadCollision-' + str(self.doId))
        ...
        self.arm1CollNode = CollisionNode('ToonfestCogArm1Collision-' + str(self.doId))
        ...
        self.arm2CollNode = CollisionNode('ToonfestCogArm2Collision-' + str(self.doId))
        ...
        self.hole.setZ(self.hole.getZ() + 0.1)
    ...
    def enterDown(self):
        ...
        self.hitInterval = Sequence(LerpScaleInterval(self.hole, duration=0.175, scale=endScale, startScale=startScale, blendType='easeIn'), Parallel(SoundInterval(self.upSound, volume=0.6, node=self.actor, cutOff=PartyGlobals.PARTY_COG_CUTOFF), ActorInterval(self.actor, 'down', loop=0)), Func(self.actor.stash), LerpScaleInterval(self.hole, duration=0.175, scale=Point3(3, 3, 3), startScale=endScale, blendType='easeOut'))
    ...
    def enterUp(self):
        ...
        startScale = self.hole.getScale()
        endScale = Point3(5, 5, 5)
        self.hitInterval = Sequence(LerpScaleInterval(self.hole, duration=0.175, scale=endScale, startScale=startScale, blendType='easeIn'), Func(self.actor.unstash), Parallel(SoundInterval(self.upSound, volume=0.6, node=self.actor, cutOff=PartyGlobals.PARTY_COG_CUTOFF), ActorInterval(self.actor, 'up', loop=0)), Func(self.actor.loop, 'idle'), LerpScaleInterval(self.hole, duration=0.175, scale=Point3(3, 3, 3), startScale=endScale, blendType='easeOut'))
        self.hitInterval.start()
    ...
    def respondToPieHit(self, timestamp, position, hot = False, direction = 1.0):
        if self.netTimeSentToStartByHit < timestamp:
            ...
            self.sendUpdate('updateTower', [avName, self.toon.doId, self.doId])
        else:
