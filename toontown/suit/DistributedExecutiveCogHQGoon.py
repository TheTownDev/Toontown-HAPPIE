...
from .DistributedCogHQEntity import DistributedCogHQEntity
...
class DistributedExecutiveCogHQGoon(DistributedCogHQEntity, Goon.Goon, FSM.FSM):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedExecutiveCogHQGoon')

    def __init__(self, cr):
        try:
            self.DistributedExecutiveCogHQGoon_initialized
        except:
            self.DistributedExecutiveCogHQGoon_initialized = 1
            DistributedCogHQEntity.__init__(self, cr)
            Goon.Goon.__init__(self)
            FSM.FSM.__init__(self, 'DistributedExecutiveCogHQGoon')
    ...
    def announceGenerate(self):
        DistributedCogHQEntity.announceGenerate(self)
    ...
    def generate(self):
        DistributedCogHQEntity.generate(self)
    ...
    def setPath(self, xi, yi, zi, xf, yf, zf):
        self.path = ((xi, yi, zi), (xf, yf, zf))
    ...
    def makePathTrack(self, node, velocity, name, turnTime=1, lookAroundNode=None):
        ...
        if self.path is None:
            ...
            return track
        startPoint = (self.path[0][0], self.path[0][1], self.path[0][2]) * self.pathScale
        endPoint = (self.path[1][0], self.path[1][1], self.path[1][2]) * self.pathScale
        ...
        track.append(LerpPosInterval(node, duration=duration, pos=(endPoint[0], endPoint[1], endPoint[2]),
                                     startPos=(startPoint[0], startPoint[1], startPoint[2])))
        curDuration = track.getDuration()
        theta = node.getH() % (-180 if self.reverseTheta == True else 180)
        track.append(LerpHprInterval(node, turnTime, Vec3(theta, 0, 0)))
        track.append(LerpPosInterval(node, duration=duration, pos=(startPoint[0], startPoint[1], startPoint[2]),
                                     startPos=(endPoint[0], endPoint[1], endPoint[2])))

        return track

    def disable(self):
        self.notify.debug('DistributedExecutiveCogHQGoon %d: disabling' % self.getDoId())
        self.ignoreAll()
        self.stopToonDetect()
        ...
        if self.walkTrack:
            ...
            self.walkTrack = None
        DistributedCogHQEntity.disable(self)
        return
    ...
    def delete(self):
        try:
            ...
            self.notify.debug('DistributedExecutiveCogHQGoon %d: deleting' % self.getDoId())
            ...
            DistributedCogHQEntity.delete(self)
