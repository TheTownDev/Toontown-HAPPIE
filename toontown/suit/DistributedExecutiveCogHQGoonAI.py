...
class DistributedExecutiveCogHQGoonAI(DistributedCogHQEntityAI):
    UPDATE_TIMESTAMP_INTERVAL = 180.0
    STUN_TIME = 4
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedExecutiveCogHQGoonAI')
    ...
    def delete(self):
        ...
        DistributedCogHQEntityAI.delete(self)

    def generate(self):
        self.notify.debug('generate')
        DistributedCogHQEntityAI.generate(self)
    ...
    def requestBattle(self, pauseTime):
        ...
        taskMgr.remove(self.taskName('resumeWalk'))
        taskMgr.doMethodLater(5, self.sendMovie, self.taskName('resumeWalk'),
    ...
    def requestStunned(self, pauseTime):
        ...
        taskMgr.remove(self.taskName('recovery'))
        taskMgr.doMethodLater(self.STUN_TIME, self.sendMovie, self.taskName('recovery'),
    ...
    def sendMovie(self, type, avId=0, pauseTime=0):
        ...
        if type == GOON_MOVIE_WALK or type == GOON_MOVIE_SYNC:
            ...
            taskMgr.remove(self.taskName('sync'))
            taskMgr.doMethodLater(self.UPDATE_TIMESTAMP_INTERVAL, self.requestResync, self.taskName('sync'),
                                  extraArgs=None)
        else:
            self.sendUpdate('setMovie', [type,
                                         avId,
    ...
    def doCrush(self, crusherId, axis):
        ...
        DistributedCogHQEntityAI.doCrush(self, crusherId, axis)
        self.crushed = 1
        self.grid.removeObject(self.entId)
    ...
    def d_setPath(self, xi, yi, zi, xf, yf, zf):
        self.sendUpdate('setPath', [xi, yi, zi, xf, yf, zf])

    def d_setPathDuration(self, pathDuration):
        self.sendUpdate('setPathDuration', [pathDuration])

    def d_setReverseTheta(self, reverseTheta):
        self.sendUpdate('setReverseTheta', [reverseTheta])