...
class DistributedCogHQGoon(DistributedCogHQEntity, Goon.Goon, FSM.FSM):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedCogHQGoon')

    def __init__(self, cr):
        try:
            self.DistributedCogHQGoon_initialized
        except:
            self.DistributedCogHQGoon_initialized = 1
            DistributedCogHQEntity.__init__(self, cr)
            Goon.Goon.__init__(self)
            FSM.FSM.__init__(self, 'DistributedCogHQGoon')
    ...
    def disable(self):
        self.notify.debug('DistributedCogHQGoon %d: disabling' % self.getDoId())
        self.ignoreAll()
        self.stopToonDetect()
    ...
    def delete(self):
        try:
            ...
            self.notify.debug('DistributedCogHQGoon %d: deleting' % self.getDoId())
